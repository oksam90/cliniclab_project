...
<div class="row">
  ...
  <div class="col-lg-2 col-6">
    ...
    <div class="small-box bg-info">
      ...
      <a href="{{route('admin.appointment.index')}}" class="small-box-footer">{{__('More info')}} <i class="fas fa-arrow-circle-right"></i></a>
    </div>
  </div>
...
<div class="row">
  <div class="col-lg-12 table-responsive">
    <div class="card card-danger">
      <div class="card-header">
        <h5 class="card-title">
          <i class="fas fa-bell"></i> {{__('Today scheduled appointment')}} ( {{count($today_appointment)}} )
        </h5>
        <div class="card-tools">
          <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i>
          </button>
      ...
      <div class="card-body table-responsive">
        ...
        <table class="table table-bordered table-striped">
          ...
          <tbody>
            @foreach($today_appointment as $appointment)
            <tr>
              <td>
              ...
              <td>
                ...
                <input type="checkbox" id="change_status" appointment-id="{{$appointment['id']}}" checked netliva-switch data-active-text="{{__('Completed')}}" data-passive-text=" {{__('Pending appointment')}}" />
                @else
                <input type="checkbox" id="change_status" appointment-id="{{$appointment['id']}}" netliva-switch data-active-text="{{__('Completed')}}" data-passive-text=" {{__('Pending appointment')}}" />
                @endif
              </td>
              <td width="100px">
                @can('view_appointment')
                <a href="{{route('admin.appointment.show',$appointment['id'])}}" class="btn btn-primary btn-sm">
                  <i class="fa fa-eye"></i>
                </a>
                @endcan
              </td>
